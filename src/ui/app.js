...
function setSockDot(state) {
  ...
  dot.title =
    state === "online"
      ? "Sengar Infotech online"
      : state === "reconn"
      ? "reconnecting..."
      : "Sengar Infotech offline";
}
...
function setSessionInfo(s) {
  ...
  let device = s?.device || s?.platform || "Sengar Infotech";
  if (!device || device === "unknown" || device === "undefined")
    device = "Sengar Infotech";
  ...
  if (isOpenNow) {
    (async () => {
      try {
        ...
        let d = info.device || device || "Sengar Infotech";
        if (!d || d === "unknown" || d === "undefined") d = "Sengar Infotech";
        ...
        if (devEl) devEl.textContent = d || "Sengar Infotech";
      } catch {}
    })();
  }
  (async () => {
...
function connectSocket({ silent = false } = {}) {
  ...
  try {
    ...
    ioClient.on("welcome", ({ role, isAdmin, version }) => {
      ...
      if (titleElement) {
        titleElement.textContent.includes("{SengarInfotechVersion}") &&
          (titleElement.textContent = titleElement.textContent.replace(
            "{SengarInfotechVersion}",
...
el("btnLoad")?.addEventListener("click", () => {
  if (ioClient?.connected) {
    forceLogout("Signed out from Sengar Infotech");
    return;
  }
